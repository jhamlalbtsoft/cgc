...
if($memData['Shop'] && trim($memData['Shop'])<>"")
     $FirmName .= ', '.$memData['Shop'];
if($memData['Complex'] && trim($memData['Complex'])<>"")
     $FirmName .= ', '.$memData['Complex'];
if($memData['Street'] && trim($memData['Street'])<>"")
     $FirmName .= ', '.$memData['Street'];

 $addr='';
if($memData['AreaName'])
     $addr = ' '.$memData['AreaName'];
if($memData['CityName'])
        $addr .= ' '.$memData['CityName'];
...
<div style ="z-index:100000;position:absolute  ;top:38.3%;left:25%"><?= $RegistrationNo ?></div>
<div style ="z-index:100000;position:absolute  ;top:38.3%;left:79%"> <?= $memData['GroupsId']-100 ?>  </div><div style ="z-index:100000;position:absolute  ;top:51.3%;left:25%" class="name">&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <?= $Representative ?></div>
  
  
     
 <div style ="z-index:100000;position:absolute  ;top:54.5%;left:24%" class="name"><?= $FirmName?> </div>      
 <div style ="z-index:100000;position:absolute  ;top:57%;left:24%" class="name"><?= $addr?> </div>      
            
<div style ="z-index:100000;position:absolute  ;top:78%;left:15%" class="bottom">Issued By:  admin, &nbsp  Signature  </div><!--div style ="z-index:100000;position:absolute  ;top:103%;left:5%; font-size:12px; color:Black; font-weight:normal">Registration No:  <?= $memData['RegistrationNoOld'] ?>  </div-->
</div>  </body></html>
