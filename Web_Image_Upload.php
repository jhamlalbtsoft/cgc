...
if(isset($_FILES['FileUpload1'])){
	...
	if (move_uploaded_file($_FILES['FileUpload1']['tmp_name'], $uploadfile)) {
	  //echo "success<br>";
	  echo $_SESSION[$ImageRep] =  'Image/'.basename($_FILES['FileUpload1']['name']);
...
<body style="background-color:Transparent; background-image:none">
    <form name="form1" method="post" action="./Web_Image_Upload.php?size=50" id="form1" enctype="multipart/form-data">
<div>
...
<div>
    ...
    <div>
        ...
        <table id="TdImg" width="100%" style="background-color:Transparent; font-family: calibri;">
	<tr>
		<td style="text-align: right; height: 22px;width:74px">
                    Image</td>
		<td style="height: 22px;">
                    <input type="file" name="fuphoto" id="fuphoto" class="btn" style="width:270px;" />
        <input type="submit" name="ButtonSubmit" value="Add" id="ButtonSubmit" class="btn" />
                    </td>
	</tr>
</table>
	...
	<tr>
        ...
        <input type="submit" name="Button1" value="Add" id="Button1" class="btn" />
                    </td>
	</tr>
</table>
  ...
  var fuphoto = $("#fuphoto");
      $('#fuphoto').bind('change', function () {
            if((this.files[0].size / 1024) > parseInt(50))
            {
