...
//     $update = $pdo->prepare("UPDATE members_edit SET otp = ? WHERE MembersId = ?");
//     $update->execute([$otp, $user['MembersId']]);
// } else {
    // Insert new user with OTP
    $insert = $pdo->prepare("INSERT INTO members_edit (MobileRep1, otp,member_id) VALUES (?, ?, ?)");
    $insert->execute([$mobile, $otp, $memberId]);
// }
