...
if(isset($_FILES['FileUploadImg'])){
	...
	if(isset($_REQUEST['id'])){
		$eepImage = $_REQUEST['id'];//18
	}

	$uploaddir = 'Image/merged/';
	$uploadfile = $uploaddir . basename($_FILES['FileUploadImg']['name']);
	//echo basename($_FILES['FileUploadImg']['name']);

	if (move_uploaded_file($_FILES['FileUploadImg']['tmp_name'], $uploadfile)) {
	  //echo "success<br>";
	  echo $_SESSION[$eepImage] =  'Image/merged/'.basename($_FILES['FileUploadImg']['name']);
	} else {
	  echo "errors";
	}
  ...
  <div class="row"style="padding:0 0px">
    <div class="span12"style="padding:0 0px">
      <div class="row-fluid column-seperation" style="padding:0 10px">
        ...
        <div class="span5">
    <form name="form1" method="post" action="./Web_Image_eep.php?File=<?= $_REQUEST['File'] ?>&amp;id=<?= $_REQUEST['id'] ?>&amp;size=150" id="form1" enctype="multipart/form-data">
    <div>
	...
	<tr>
		<td>
                    <input type="file" name="FileUploadImg" id="FileUploadImg" style="height:30px;width:200px;" />
...
<script src="./assets/js/login.js" type="text/javascript"></script>
  ...
  var fuphoto = $("#fuphoto");
      $('#fuphoto').bind('change', function () {
       ...
       var FileUploadImg = $("#FileUploadImg");
      $('#FileUploadImg').bind('change', function () {
          
            if((this.files[0].size / 1024) > parseInt(1500))
            {
                 alert('File Size exceed the permited limit of  1500  KB. Please reduce the file size. This file size is: ' + this.files[0].size / 1024  + "KB");
                  //FileUploadImg.replaceWith( FileUploadImg = FileUploadImg.clone( true ) );
                  $('#file_reset').trigger('click');
            }
